...
int min_coin_change(int coin, vector<int> & denomination, int n){
	...
	for(int i=0;i<n;i++){
		...
		if(coin-denomination[i] >= 0)
			ans=min(ans, 1+min_coin_change(coin-denomination[i], denomination, n) );  
	}
...
void solve()
{
	...
	cout<<min_coin_change(coin, denomination, n)<<endl; 
}
int32_t main(){
#ifndef ONLINE_JUDGE
